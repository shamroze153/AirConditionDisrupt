...
const GlobalDashboardView: React.FC<Props> = ({ stats }) => {
  ...
  // Data Normalization with robust year filtering
  ...
  const logs = useMemo(() => {
    return (stats?.allPerformanceLogs || []).filter(l => {
      const d = parseHubDate(l.Timestamp);
      return d && d.getFullYear() === CURRENT_YEAR;
    });
  }, [stats, CURRENT_YEAR]);
  ...
  const kpiMetrics = useMemo(() => {
    ...
    const resolved = tickets.filter(t => ['Resolved', 'Resolved (Admin)', 'Resolved by Technician', 'Completed'].includes(t.status)).length;
