...
const TechView: React.FC<Props> = ({ category, attendance, toggleAttendance, tickets, assets, onOpenChecklist, showToast, onRefresh, stats }) => {
  ...
  useEffect(() => {
    if (activeTechList.length > 0 && !selectedTech) setSelectedTech(activeTechList[0]);
  ...
  const handleResolve = async () => {
    ...
    try {
      await postAction(fd);
      await updatePoints(category, selectedTech, 2, "Ticket Resolution Bonus");
      onRefresh();
      setResolveTicket(null);
      setResolveRemarks('');
      showToast("Resolution Recorded (+2 Merit)");
  ...
  return (
    <div className="max-w-[1400px] mx-auto p-4 md:p-6 space-y-6 animate-fadeIn pb-32">
        ...
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="flex gap-3 overflow-x-auto pb-4 hide-scroll px-1">
             {activeTechList.map(t => (
               <button 
                 ...
                 className={`group flex items-center gap-4 p-4 rounded-3xl border-2 transition-all min-w-[180px] md:min-w-[220px] ${selectedTech === t ? 'bg-slate-950 border-slate-950 shadow-2xl scale-105' : 'bg-white border-slate-100 opacity-60 hover:opacity-100 hover:border-slate-200 shadow-sm'}`}
               >
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-xl italic shadow-inner ${selectedTech === t ? 'bg-indigo-600 text-white' : 'bg-slate-50 text-slate-300 group-hover:bg-slate-100'}`}>
                    {t[0]}
                  </div>
                  <div className="text-left">
                    <p className={`text-[10px] font-black uppercase tracking-widest italic leading-none ${selectedTech === t ? 'text-white' : 'text-slate-900'}`}>{t}</p>
                    <p className={`text-[7px] font-bold uppercase mt-1.5 italic ${selectedTech === t ? 'text-white/40' : 'text-slate-300'}`}>
          ...
          {selectedTech ? (
                 ...
                 <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-xl relative overflow-hidden group">
                    ...
                    <div className="mt-8 pt-8 border-t border-slate-50 relative z-10">
                       <div className="flex justify-between items-center mb-4">
                          <span className="text-[9px] font-black text-slate-400 uppercase italic tracking-widest">Attendance Toggle</span>
                          <button 
            ...
            <div className="py-32 text-center opacity-10 flex flex-col items-center">
               <i className="fas fa-user-astronaut text-8xl mb-8"></i>
               <p className="text-xl font-black uppercase tracking-[0.5em] italic">Select Specialist Operative</p>
            </div>
          )}
        </div>
      )}
               ...
               <div className={`grid grid-cols-1 ${category === 'handyman' ? 'sm:grid-cols-1' : 'sm:grid-cols-2'} gap-4 md:gap-6`}>
                  {activeTechList.map((tech, i) => (
                    <button key={i} onClick={() => onOpenChecklist(i, tech)} className="bg-slate-50/50 p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] border border-slate-100 flex flex-col hover:bg-white hover:shadow-2xl transition-all group active:scale-[0.98] text-left relative overflow-hidden">
                      ...
                      <div className="mt-auto pt-6 border-t border-slate-100 flex gap-4 opacity-40 group-hover:opacity-100 transition-opacity">
                         <span className="text-[8px] font-black uppercase italic tracking-widest">Protocol Verification Active</span>
                      </div>
                    </button>
                  ))}
               </div>
                 ...
                 <div className="space-y-6">
                    ...
                    <div className="bg-slate-50 p-6 rounded-[2rem] border-2 border-slate-100 focus-within:border-amber-500 transition-all">
                       ...
                       <textarea 
                         ...
                         rows={4} 
                         placeholder="Detail required components, quantities, and justification..." 
                         className="w-full bg-transparent font-bold text-base outline-none italic uppercase resize-none placeholder:text-slate-200 leading-relaxed"
                       />
                    </div>

                    <button 
                      onClick={handleSubmitDemand} 
                      disabled={isSubmittingDemand || !demandText.trim()} 
                      className="w-full bg-slate-950 text-white py-8 rounded-[2rem] font-black uppercase text-[11px] tracking-[0.4em] shadow-2xl active:scale-95 transition-all disabled:opacity-30 italic flex items-center justify-center gap-4"
                    >
                       ...
                       <span>{isSubmittingDemand ? 'Synchronizing Supply Hub...' : 'Authorize Material Request'}</span>
                    </button>
                 </div>
              </div>
           </div>
        ...
        <div className="animate-fadeIn space-y-6">
              ...
              <div className="flex items-center gap-3 w-full md:w-auto">
                 <div className="relative flex-1 md:w-64">
                   <input 
                     ...
                     className="w-full bg-white border border-slate-100 px-10 py-3 rounded-xl text-[10px] font-bold outline-none focus:border-indigo-500 shadow-sm"
                   />
                   <i className="fas fa-search absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 text-[10px]"></i>
                 </div>
                 <button 
                   onClick={handleAdminToggle} 
                   className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all ${isAdminUnlocked ? 'bg-indigo-600 text-white shadow-lg' : 'bg-white text-slate-300 border border-slate-100 hover:text-indigo-600'}`}
                 >
           ...
           {isLoadingTools ? (
             <div className="py-32 flex flex-col items-center justify-center opacity-10">
                <i className="fas fa-circle-notch animate-spin text-6xl mb-4"></i>
                <p className="text-xs font-black uppercase tracking-widest italic">Fetching Tool Registry...</p>
             </div>
           ) : (
                ...
                {serverTools.filter(t => !toolSearch || t.name.toLowerCase().includes(toolSearch.toLowerCase())).map((tool, i) => (
                  <div key={i} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all group relative overflow-hidden">
                     <div className="flex justify-between items-start mb-6">
                        <div className="w-10 h-10 bg-slate-50 text-slate-400 rounded-xl flex items-center justify-center group-hover:bg-slate-900 group-hover:text-white transition-all text-sm">
                     ...
                     {isAdminUnlocked && (
                       <div className="flex gap-2 pt-4 border-t border-slate-50">
                          <button 
                            onClick={() => handleUpdateToolQty(tool.name, -1)} 
                            className="flex-1 bg-slate-50 text-slate-400 py-3 rounded-xl hover:bg-rose-50 hover:text-rose-600 transition-all active:scale-95"
                          >
                             <i className="fas fa-minus text-[10px]"></i>
                          </button>
                          <button 
                            onClick={() => handleUpdateToolQty(tool.name, 1)} 
                            className="flex-1 bg-slate-50 text-slate-400 py-3 rounded-xl hover:bg-emerald-50 hover:text-emerald-600 transition-all active:scale-95"
                          >
                             <i className="fas fa-plus text-[10px]"></i>
                          </button>
                       </div>
                     )}
                     <div className="absolute -bottom-1 -right-1 w-12 h-12 bg-slate-950/5 blur-xl group-hover:bg-indigo-500/10 transition-all"></div>
                  </div>
                ))}
             </div>
           )}
      ...
      {resolveTicket && (
        <div className="fixed inset-0 bg-slate-950/95 z-[600] flex items-center justify-center p-6 backdrop-blur-3xl animate-fadeIn">
          <div className="bg-white w-full max-w-md rounded-[3rem] p-10 shadow-3xl border border-white/5 relative overflow-hidden">
             <div className="flex justify-between items-center mb-10">
               <div>
                 <h3 className="text-2xl font-black text-slate-950 leading-none italic uppercase tracking-tighter">Resolution Protocol</h3>
                 <p className="text-[9px] font-bold text-slate-400 uppercase mt-2 tracking-widest italic">Unit Identification: {resolveTicket.assetTag}</p>
               </div>
             ...
             <div className="space-y-6">
                ...
                <button onClick={handleResolve} disabled={isResolving || !resolveRemarks.trim()} className="w-full bg-slate-950 text-white py-6 rounded-[1.5rem] font-black uppercase text-[10px] tracking-[0.4em] shadow-2xl active:scale-95 transition-all disabled:opacity-30 italic flex items-center justify-center gap-4">
                  {isResolving ? <i className="fas fa-circle-notch animate-spin text-teal-400"></i> : <i className="fas fa-check-double text-teal-400"></i>}
                  <span>{isResolving ? 'Synchronizing...' : 'Finalize Resolution'}</span>
                </button>
             </div>
          </div>
        </div>
