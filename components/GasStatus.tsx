...
const GasStatus: React.FC<Props> = ({ stats, onRefresh }) => {
  ...
  return (
    <div className="relative w-full">
      <div onClick={handleTrigger} className="grid grid-cols-5 gap-4 cursor-pointer">
        {GAS_TYPES.map((g: any, i: number) => {
          ...
          const colorClass = g.type === 'ac' ? 'from-indigo-400 to-indigo-700' : 'from-slate-400 to-slate-600';
          
          return (
            <div key={i} className="flex flex-col items-center">
              <div className="w-full h-48 bg-white/5 rounded-t-[3rem] rounded-b-2xl relative overflow-hidden flex items-end border-2 border-white/10 shadow-2xl group transition-all hover:scale-105 active:scale-95">
